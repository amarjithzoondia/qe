//
//  NFRequestAccessViewModel.swift
//  QualityExpertise
//
...
class NFRequestAccessViewModel: ObservableObject {
    ...
    func viewGroup(groupId: String, groupShortCode: String, notificationId: Int, completion: @escaping (_ completed: Bool) -> ()) {
        do {
            let groupCode = try groupShortCode.validatedText(validationType: .requiredField(field: "project_code".localizedString()))
    ...
    func requestToGroup(groupId: String, groupShortCode: String,completion: @escaping (_ completed: Bool) -> ()) {
        do {
            let groupId = try groupId.validatedText(validationType: .requiredField(field: "project_id".localizedString()))
            let groupCode = try groupShortCode.validatedText(validationType: .requiredField(field: "project_code".localizedString()))
