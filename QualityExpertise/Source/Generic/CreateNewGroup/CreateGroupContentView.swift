...
struct CreateGroupContentView: View {
    ...
    var body: some View {
        NavigationView {
            ...
            .navigationBarTitle(viewModel.isEditing ? "edit_group".localizedString() : "new_group".localizedString(), displayMode: .inline)
            .toolbar {
    ...
    var titleView: some View {
        ThemeTextEditorView(
            text: $title,
            title: "group_title".localizedString(),
            disabled: false,
            keyboardType: .default,
            isMandatoryField: false,
            limit: Constants.Number.Limit.GROUP_NAME,
            placeholderColor: Color.Grey.GUNMENTAL.opacity(0.25),
            isTitleCapital: true
        )
    }
    ...
    func onCreateAndContinue() {
        ...
        viewModel.createGroup(title: title, description: description, image: imageUrl) { completed in
            if viewModel.isEditing {
                ...
                viewControllerHolder?.present(style: .overCurrentContext) {
                    GroupCreationSuccessContentView(viewModel: .init(groupdetails: viewModel.groupDetails!))
                        .localize()
                }
            }
        }
...
struct CreateGroupContentView_Previews: PreviewProvider {
