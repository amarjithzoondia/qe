...
struct UserListContentView: View {
            ...
            ZStack {
                ...
                VStack {
                    SearchFieldInputView(
                        ...
                        text: $searchText,
                        placeholder: "search_user".localizedString(),
                    ...
                    VStack {
                        if viewModel.isLoading {
                            ...
                            "no_user_found".localizedString().viewRetry {
                                viewModel.onRetry()
                            }
                            ...
                            if viewModel.searchUsersList.count == 1 && forAvoidUser {
                                ...
                                ScrollView(showsIndicators: false) {
                                    if isFromSelectAUser && !isFromViewGroup {
                                        VStack(spacing: 20) {
                                            ...
                                            VStack(spacing: 15) {
                                                VStack {
                                                    HStack{
                                                        Text("custom_entry".localizedString())
                                                            .foregroundColor(Color.Indigo.DARK)
                                                            .font(.medium(15))
                                    ...
                                    LeftAlignedHStack(
                                        Text("user_list".localizedString())
                    ...
                    Button(action: {
                        if isFromSelectAUser {
                            ...
                            if (userData == nil || viewModel.searchUsersList.allSatisfy({$0.isSelected == false})) && !isAllGroupMembersSelected && customResponsiblePersonName == "" {
                                viewModel.toast = SystemError("select_one_user_alert".localizedString()).toast
                            } else if customResponsiblePersonEmail != "" && !customResponsiblePersonEmail.isValidEmail() {
                                customResponsiblePersonEmail = ""
                                viewModel.toast = SystemError("enter_valid_mail".localizedString()).toast
                            } else {
                                if isCustomResponsiblePersonSelected {
    ...
    var customResponsiblePersonNameView: some View {
        ThemeTextEditorView(
            ...
            placeholder: "responsible_person_name".localizedString(), disabled: isCustomResponsiblePersonSelected ? false : true,
    ...
    var customResponsiblePersonEmailView: some View {
        ThemeTextEditorView(
            ...
            title: "".localizedString(),
            placeholder: "email_address_optional".localizedString(),
...
struct UserListContentView_Previews: PreviewProvider {
    static var previews: some View {
        UserListContentView(viewModel: .init(groupData: GroupData.dummy()), isUserActive: .constant(false), isUserActiveForNotification: .constant(false), userData: .constant(UserData.dummy()), userDatas: .constant([UserData.dummy()]))
    }
}
