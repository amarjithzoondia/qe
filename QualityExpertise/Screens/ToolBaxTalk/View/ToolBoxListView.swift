...
struct ToolBoxListView: View {
    ...
    var body: some View {
        NavigationView {
            ZStack {
                ...
                VStack(spacing: 0) {
                    // Header with Add New button
                        ...
                        HStack {
                            ...
                            .onTapGesture {
                                viewControllerHolder?.present(style: .overCurrentContext) {
                                    DraftToolBoxListContentView(onNewIncidentAdded: {
                                        viewModel.resetPagination()
                                        fetchList(isInital: true)
                                    })
                                    .localize()
                        ...
                        SearchFieldInputView(
                            ...
                            text: $searchText,
                            placeholder: "search_toolbox_talks".localizedString(),
                ...
                if viewModel.searchToolBoxTalks.count > 0 {
                    VStack {
                        Spacer()
            ...
            .navigationBarTitle("toolbox_talk_title".localizedString(), displayMode: .inline)
            .toolbar {
                ...
                ToolbarItem(placement: .navigationBarTrailing) {
                    HStack(spacing: 15) {
                        Button {
                            ...
                            Button {
                                viewControllerHolder?.present(style: .overCurrentContext) {
                                    FilterSectionContentView(noProjectSpecified: $noProjectSpecified, startDate: $startDate, endDate: $endDate, isActive: $isActive, selectedProjectIds: $projectsIds, selectedResponsibleIds: $reportedByIds, selectedIncidentIds: .constant([]), title: "toolbox_talk".localizedString())
                                        .localize()
                                }
                            } label: {
                                ZStack {
            ...
            if viewModel.noDataFound {
                ...
                ScrollView {
                    LazyVStack(spacing: 12) {
                        ForEach(viewModel.searchToolBoxTalks.indices, id: \.self) { index in
                            let toolBoxTalk = viewModel.searchToolBoxTalks[index]
                            ToolBoxListRowView(toolBoxTalk: toolBoxTalk)
                            .onTapGesture {
                                viewControllerHolder?.present(style:.overCurrentContext) {
                                    CreateToolBoxView(
                            ...
                            .onAppear {
                                if viewModel.searchToolBoxTalks.count - 1 == index {
                                    fetchList()
                                }
                            }
                        }
    ...
    private func fetchList(isInital: Bool = false) {
        viewModel.fetchToolBoxList(searchText: searchText, sortType: isListSorted ? .ascending : .descending, isInital: isInital, selectedProjectsIds: projectsIds, openDate: startDate, endDate: endDate, reportedByPersonsIds: reportedByIds, noProjectSpecified: noProjectSpecified)
    }
    ...
    private func saveExcel(urlString: String) {
        ...
        let fileName = "ALNASR - ToolBox Talks Log \(Date().formattedDateString(format: "dd.MM.yyyy")).csv"
