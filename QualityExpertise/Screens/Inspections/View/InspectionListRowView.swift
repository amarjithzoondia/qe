...
struct InspectionListRowView: View {
    var inspections: Inspections
    let viewModel: InspectionsListRowViewModel
    var body: some View {
        VStack {
            VStack(spacing: 11) {
                HStack {
                    Text(inspections.auditItem.auditItemTitle)
                ...
                if let facilities = inspections.facilities {
                    HStack(spacing: 6) {
                ...
                if inspections.images?.count ?? 0 > 0 {
                    HStack(spacing: 9) {
                        if inspections.images?.count ?? 0 == 1 {
                            WebUrlImage(url: inspections.images?[0].image?.url ?? "".url)
                                .frame(height: 97)
                                .cornerRadius(10)
                                .clipped()
                        } else {
                            ...
                            VStack {
                                WebUrlImage(url: inspections.images?[1].image?.url ?? "".url)
