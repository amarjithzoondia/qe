...
struct PreTaskListView: View {
    ...
    @StateObject private var viewModel = PreTaskListViewModel()
    ...
    var body: some View {
        NavigationView {
            ZStack {
                ...
                VStack(spacing: 0) {
                    // Header with Add New button
                        ...
                        HStack {
                            ...
                            .onTapGesture {
                                viewControllerHolder?.present(style: .overCurrentContext) {
                                    DraftPreTaskListContentView(onNewPreTaskAdded: {
                                        viewModel.resetPagination()
                                        fetchList(isInital: true)
                                    })
                                    .localize()
                        ...
                        SearchFieldInputView(
                            ...
                            text: $searchText,
                            placeholder: "search_pre_task".localizedString(),
                        ...
                        .onChange(of: isListSorted, perform: { (value) in
                            viewModel.sortToolBoxTalks(searchText: searchText, sortType: isListSorted ? .ascending : .descending, selectedProjectsIds: projectsIds, openDate: startDate, endDate: endDate, reportedByPersonsIds: reportedByIds, noProjectSpecified: noProjectSpecified)
                        })
                ...
                if viewModel.searchPreTasks.count > 0 {
                    VStack {
                        Spacer()
            ...
            .navigationBarTitle("pre_task".localizedString(), displayMode: .inline)
            .toolbar {
                ...
                ToolbarItem(placement: .navigationBarTrailing) {
                    HStack(spacing: 15) {
                        Button {
                            ...
                            Button {
                                viewControllerHolder?.present(style: .overCurrentContext) {
                                    FilterSectionContentView(noProjectSpecified: $noProjectSpecified, startDate: $startDate, endDate: $endDate, isActive: $isActive, selectedProjectIds: $projectsIds, selectedResponsibleIds: $reportedByIds, selectedIncidentIds: .constant([]), title: "pre_task".localizedString())
                                        .localize()
                                }
                            } label: {
                                ZStack {
            ...
            if viewModel.noDataFound {
                ...
                ScrollView {
                    LazyVStack(spacing: 12) {
                        ForEach(viewModel.searchPreTasks.indices, id: \.self) { index in
                            let preTasks = viewModel.searchPreTasks[index]
                            PreTaskListRowView(preTask: preTasks)
                            .onTapGesture {
                                viewControllerHolder?.present(style:.overCurrentContext) {
                                    CreatePreTaskView(
                                        preTask: preTasks,
                            ...
                            .onAppear {
                                if viewModel.searchPreTasks.count - 1 == index {
                                    fetchList()
                                }
                            }
                        }
    ...
    private func fetchList(isInital: Bool = false) {
        viewModel.fetchPreTaskList(searchText: searchText, sortType: isListSorted ? .ascending : .descending, isInital: isInital, selectedProjectsIds: projectsIds, openDate: startDate, endDate: endDate, reportedByPersonsIds: reportedByIds, noProjectSpecified: noProjectSpecified)
    }
    ...
    private func saveExcel(urlString: String) {
        ...
        let fileName = "ALNASR - Pre-Task Briefings Log \(Date().formattedDateString(format: "dd.MM.yyyy")).csv"
...
struct PreTaskListView_Previews: PreviewProvider {
