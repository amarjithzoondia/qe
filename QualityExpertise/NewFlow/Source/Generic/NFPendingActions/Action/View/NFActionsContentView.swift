...
struct NFActionsContentView: View {
    @Environment(\.layoutDirection) private var layoutDirection
    @Environment(\.viewController) private var viewControllerHolder: UIViewController?
    @StateObject var viewModel = NFActionsViewModel()
    @State var selectedItem: NFPendingActionType = .openObservation
    @Binding var pendingAction: NFPendingActionDetails
                ...
                VStack {
                    if pendingAction.type == .openObservation {
                        openObservationView
                            ...
                            .padding(.bottom, 20)
                    } else if pendingAction.type == .requestToJoinProject {
            ...
            VStack(spacing: 20) {
                ...
                .onTapGesture {
                    viewControllerHolder?.present(style: .overCurrentContext) {
                        NFObservationDetailContentView(viewModel: .init(observationId: pendingAction.contentId))
                            .localize()
                    }
                }
                ...
                .onTapGesture {
                    viewControllerHolder?.present(style: .overCurrentContext) {
                        NFCloseObservationContentView(viewModel: .init(observationId: pendingAction.contentId, observationTitle: "", observationDescription: "", imageDescription: [], groupSpecified: false), isfromPendingAction: true)
                            .localize()
                    }
                }
                ...
                .onTapGesture {
                    ...
                    viewControllerHolder?.present(style: .overCurrentContext) {
                        NFResponsiblePersonChangeContentView(viewModel: .init(groupData: GroupData(groupId: DataComponents.last ?? "", groupCode: pendingAction.groupCode, groupName: "", groupImage: "", userRole: Optional.none, isSelected: false, description: ""), observationId: pendingAction.contentId))
                            .localize()
                    }
                }
                ...
                .onTapGesture {
                    viewControllerHolder?.present(style: .overCurrentContext) {
                        NFDeleteObservationRequestContentView(viewModel: .init(observationId: pendingAction.contentId))
                            .localize()
                    }
                }
            }
    ...
    var requestToJoinGroupView: some View {
        VStack(spacing: 25) {
            LeftAlignedHStack(
                Text("request_to_join_project".localizedString())
                    .foregroundColor(Color.Indigo.DARK)
                    .font(.semiBold(17.5))
            )
                .padding(.horizontal, 27)
            ...
            VStack(spacing: 20) {
                ...
                HStack {
                    Text("view_project".localizedString())
                        .foregroundColor(Color.Indigo.DARK)
                        .font(.regular(14))
                ...
                .onTapGesture {
                    let DataComponents = pendingAction.groupCode.components(separatedBy: "-")
                    viewControllerHolder?.present(style: .overCurrentContext) {
                        NFGroupDetailsContentView(viewModel: .init(groupId: DataComponents.last ?? "", groupCode: pendingAction.groupCode))
                            .localize()
                    }
                }
            }
    ...
    var reviewObservationCloseOutView: some View {
        VStack(spacing: 25) {
            if isEditable {
                ...
                VStack(spacing: 20) {
                    HStack {
                        Text("view_observation_closeout".localizedString())
                            .foregroundColor(Color.Indigo.DARK)
                            .font(.regular(14))
