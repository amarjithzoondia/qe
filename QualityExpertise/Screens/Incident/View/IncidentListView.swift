...
struct IncidentListView: View {
    ...
    var body: some View {
        NavigationView {
            ZStack {
                ...
                VStack(spacing: 0) {
                    ...
                    SearchFieldInputView(
                        ...
                        text: $searchText,
                        placeholder: "search_incidents".localizedString(),
                    ...
                    .onChange(of: isListSorted, perform: { (value) in
                        viewModel.sortIncidents(searchText: searchText, sortType: isListSorted ? .ascending : .descending, selectedProjectsIds: projectsIds, openDate: startDate, endDate: endDate, reportedByPersonsIds: reportedByIds, incidentTypesIds: incidentTypeIds, noProjectSpecified: noProjectSpecified)
                    })
            ...
            .navigationBarTitle("incidents_title".localizedString(), displayMode: .inline)
            .toolbar {
                ...
                ToolbarItem(placement: .navigationBarTrailing) {
                    HStack(spacing: 15) {
                        Button {
                            ...
                            Button {
                                viewControllerHolder?.present(style: .overCurrentContext) {
                                    FilterSectionContentView(noProjectSpecified: $noProjectSpecified, startDate: $startDate, endDate: $endDate, isActive: $isActive, selectedProjectIds: $projectsIds, selectedResponsibleIds: $reportedByIds,
                                                             selectedIncidentIds: $incidentTypeIds, isIncidentType: true, title: "incidents_title".localizedString())
                                    .localize()
                                }
                            } label: {
                                ZStack {
    ...
    private func fetchList(isInital: Bool = false) {
        viewModel.fetchIncidentsList(searchText: searchText, sortType: isListSorted ? .ascending : .descending, isInitial: isInital, selectedProjectsIds: projectsIds, openDate: startDate, endDate: endDate, reportedByPersonsIds: reportedByIds, incidentTypesIds: incidentTypeIds, noProjectSpecified: noProjectSpecified)
    }
    
    private func exportToExcel() {
        viewModel.exportToExcel(searchKey: searchText ,sortType: isListSorted ? .ascending : .descending, selectedProjectsIds: projectsIds, openDate: startDate, endDate: endDate, reportedByPersonsIds: reportedByIds, incidentTypesIds: incidentTypeIds, noProjectSpecified: noProjectSpecified) {
    ...
    private func saveExcel(urlString: String) {
        ...
        let fileName = "ALNASR - Incidents Log \(Date().formattedDateString(format: "dd.MM.yyyy")).csv"
