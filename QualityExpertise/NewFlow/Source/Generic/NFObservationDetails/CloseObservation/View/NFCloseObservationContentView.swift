//
//  NFCloseObservationContentView.swift
//  ALNASR
//
...
struct NFCloseObservationContentView: View {
    @Environment(\.viewController) private var viewControllerHolder: UIViewController?
    @StateObject var viewModel: NFCloseObservationViewModel
    ...
    var body: some View {
        NavigationView {
                            ...
                            VStack(spacing: 0) {
                                VStack(spacing: 0) {
                                    ...
                                    if viewModel.observationDescription != "" {
                                        LeftAlignedHStack(
                                            Text(viewModel.observationDescription ?? "na".localizedString())
                                ...
                                if let images = viewModel.images, images != [] {
                                    VStack {
                                        LeftAlignedHStack(
                                            Text("observation_images".localizedString())
            ...
            .navigationBarTitle("close_observation".localizedString(), displayMode: .inline)
            .toolbar {
    ...
    var closeOutDescriptionView: some View {
        ThemeTextEditorView(
            ...
            title: "close_out_description".localizedString(),
            disabled: false,
    ...
    func addImages() {
        if imageDescription.last?.image == "" {
            viewModel.toast = Toast.alert(
                subTitle: String(format: "please_add_image_first".localizedString(), "\(imageCount)")
            )
        } else {
...
struct NFCloseObservationContentView_Previews: PreviewProvider {
    static var previews: some View {
        NFCloseObservationContentView(viewModel: .init(observationId: -1, observationTitle: "", observationDescription: "", groupImage: "", groupName: "", groupCode: "", imageDescription: [], groupSpecified: false))
    }
}
