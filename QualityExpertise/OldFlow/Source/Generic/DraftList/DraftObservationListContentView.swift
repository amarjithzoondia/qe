...
struct DraftObservationListContentView: View {
    ...
    var body: some View {
            ...
            ZStack {
                ...
                VStack {
                    if viewModel.observations.count <= 0 {
                        "No Observations found".localizedString()
                            .viewRetry {
                            viewModel.onRetry()
                        }
                    } else {
        ...
        .pickerViewerOverlay(viewerShown: $minusButtonTapped, title: "Delete Draft ?") {
            VStack {
                LeftAlignedHStack(
                    Text("Are you sure you want to delete this observation draft. This observation will be removed permanently.")
