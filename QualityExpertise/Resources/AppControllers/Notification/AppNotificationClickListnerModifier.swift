            ...
            if let pushType = notification.pushType {
                ...
                case .notification:
                    ...
                    if let listType = notification.typeRaw {
                     ...
                     case .joinGrouprequestAcceptedNF, .groupMemberRoleChangedNF, .addedToGroupNF:
                         if let dataComponents = notification.groupCode?.components(separatedBy: "-") {
                             return AnyView(
                                 NFGroupDetailsContentView(viewModel: .init(groupId: dataComponents.last ?? "", groupCode: notification.groupCode ?? "", notificationId: notification.id))
                             )
                         }
                     case .joinGrouprequestRejectedNF, .removeFromGroupNF:
                         return AnyView(
                             GroupRequestRejectedPopUpContentView(viewModel: .init(notification: notification))
                         )
                     case .deleteObservationRequestApprovedNF:
                         return AnyView(
                             NFObservationListContentView()
                         )
                     case .deleteObservationRequestRejectedNF, .reviewObservationCloseOutApprovedNF, .reviewObservationCloseOutRejectedNF, .reassignresponsiblePersonApprovedNF, .reassignresponsiblePersonRejectedNF, .observationCreatedNF, .observationCreatedToResponsiblePersonNF, .observationClosedToResponsiblePersonNF:
                         return AnyView(
                             NFObservationDetailContentView(viewModel: .init(observationId: notification.contentId ?? -1, notificationId: notification.id))
                         )
                     case .observationDeletedNF:
                         return AnyView(
                             NFObservationListContentView()
                         )
                     case .generalNF:
                     ...
                     case .preTaskBreifing:
                         return AnyView(
                            CreatePreTaskView(preTask: nil, preTaskId: notification.contentId, notificationId: notification.id, isViewOnly: true, onSuccess: {
                            })
                         )
                     }
                }
