//
//  NFRequestAccessContentView.swift
//  QualityExpertise
//
...
struct NFRequestAccessContentView: View {
    
    @Environment(\.viewController) private var viewControllerHolder: UIViewController?
    @StateObject var viewModel = NFRequestAccessViewModel()
    ...
    var body: some View {
            ...
            ZStack {
                ...
                ScrollView(showsIndicators: false) {
                    VStack {
                        ...
                        Text("project_access_request".localizedString())
                            .foregroundColor(Color.Indigo.DARK)
                            .font(.semiBold(21))
                            .padding(.top, 15)
                            .multilineTextAlignment(.center)
                        
                        Text("enter_code_to_join_project".localizedString())
                            .foregroundColor(Color.Indigo.DARK)
                            .font(.regular(13.5))
                            .padding(.top, 11.5)
                            .multilineTextAlignment(.center)
        ...
        .pickerViewerOverlay(viewerShown: $isViewGroup, title: viewModel.isGroupVerified ? "" : "incorrect_code".localizedString()) {
            if viewModel.isGroupVerified {
                ...
                LeftAlignedHStack(
                    Text("project_not_exist_view".localizedString())
                        .foregroundColor(Color.Indigo.DARK)
                        .font(.regular(15))
                )
        ...
        .pickerViewerOverlay(viewerShown: $isRequestSuccess, title: viewModel.isGroupVerified ? "access_requested_title".localizedString() : "incorrect_code".localizedString()) {
            if viewModel.isGroupVerified {
                LeftAlignedHStack(
                    Text("project_access_requested".localizedString())
                        .foregroundColor(Color.Indigo.DARK)
                        .font(.regular(15))
                )
                ...
                LeftAlignedHStack(
                    Text("project_not_exist_join".localizedString())
                        .foregroundColor(Color.Indigo.DARK)
                        .font(.regular(15))
                )
...
struct NFRequestAccessContentView_Previews: PreviewProvider {
