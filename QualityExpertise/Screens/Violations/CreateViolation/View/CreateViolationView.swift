...
struct CreateViolationView: View {
    internal init(violation: Violation?, draftViolation: Violation? = nil, isViewOnly: Bool = false, onSuccess: @escaping () -> Void) {
        _viewModel = StateObject(wrappedValue: CreateViolationViewModel(violation: violation, draftViolation: draftViolation))
    ...
    @State private var violationDate: Date? = nil
    ...
    @StateObject var viewModel: CreateViolationViewModel
    ...
    var body: some View {
        NavigationView {
            ZStack {
                ...
                contentView
                    .onAppear {
                        ...
                        viewModel.fetchViolationDetails() { violation in
                            if let violation {
                                DispatchQueue.safeAsyncMain {
                                    setViolation(violation: violation)
                                }
                            }
                        }
                    }
            ...
            .navigationBarTitle(isViewOnly ? "" : "violations".localizedString(), displayMode: .inline)
            .toolbar {
                ...
                ToolbarItem(placement: .topBarTrailing) {
                    if isViewOnly, let violation = viewModel.violation {
                        HStack(spacing: 5){
                            if viewModel.isDownloading {
                                ...
                                Button {
                                    generatePdf(violation: violation)
                                } label: {
        ...
        .overlay(
            DatePickerView(selectedDate: $violationDate, showDatePicker: $showDatePicker)
                .ignoresSafeArea()
        )
        .pickerViewerOverlay(viewerShown: $showSuccessAlert, title: "violation_created".localizedString()) {
            VStack {
                LeftAlignedHStack(
                    Text("violation_created_succesfully".localizedString())
                        .foregroundColor(Color.Indigo.DARK)
                        .font(.regular(14))
                )
        ...
        .overlay(ShareObservationAlertView(viewerShown: $showShareDocumentAlert) { isPDFSelected in
            if !isPDFSelected, let violation = viewModel.violation {
                let message = viewModel.getShareMessage(violation: violation)
                showActivityViewController(activities: [message])
            } else {
                if let violation = viewModel.violation {
                    viewModel.generatePdf(violation: violation) { completed in
        ...
        .pickerViewerOverlay(viewerShown: $successAlertforDraft, title: "draft_saved".localizedString()) {
            VStack {
                LeftAlignedHStack(
                    Text("violation_saved_as_draft".localizedString())
                        .foregroundColor(Color.Indigo.DARK)
                        .font(.regular(14))
                )
        ...
        .pickerViewerOverlay(viewerShown: $groupSpecifiedAlert, title: "draft_saved".localizedString()) {
            VStack {
                LeftAlignedHStack(
                    Text("violation_saved_as_draft_without_group_specs".localizedString())
                        .foregroundColor(Color.Indigo.DARK)
                        .font(.regular(14))
                )
    ...
    private var headingContent: some View {
        VStack {
            Text("violations".localizedString())
                .font(.bold(20))
    ...
    private var formView: some View {
        VStack(spacing:17) {
            Group {
                ...
                ThemeTextEditorView(text: $employeeName, title: "employee_name".localizedString(), isMandatoryField: true)
                ThemeTextEditorView(text: $employeeID, title: "employee_id".localizedString(), isMandatoryField: true)
                ThemeDatePickerView(date: $violationDate, title: "violation_date".localizedString(), placeholder: "enter_violation_date".localizedString(), showDatePicker: $showDatePicker)
                ThemeTextEditorView(text: $location, title: "location".localizedString(), isMandatoryField: false)
            ...
            if !isViewOnly {
                ButtonWithLoader(action: {createViolation()}, title: "publish".localizedString(), width: screenWidth - ( 25 * 2 ), height: 52, isLoading: .constant(false))
    ...
    private var imageView: some View {
        ForEach(imagesDatas.indices, id: \.self) { index in
            CreateViolationImageView(
                ...
                }, isViewOnly: isViewOnly, viewModel: CreateViolationImageViewModel(item: imagesDatas[index], index: index),
        ...
        VStack(spacing:10) {
            ...
            VStack(spacing:10) {
                if isViewOnly {
                    ...
                    ThemeTextEditorView(text: $description, title: "", placeholder: "", isDividerShown: false, isPlaceHolderShown: false)
                        .padding()
                }
            }
            .frame(maxWidth: .infinity)
    ...
    func createViolation() {
        viewModel.publish(employeeName: employeeName, employeeId: employeeID, violationDate: violationDate, location: location, description: description, groupData: groupData, image: imagesDatas, reportedBy: reportedBy) { completed in
    ...
    func generatePdf(violation: Violation) {
        viewModel.generatePdf(violation: violation) { completed in
            viewModel.savePdfWithProgress(urlString: viewModel.pdfUrl, fileName: "QualityExpertise - Violations(\(viewModel.violation?.id ?? -1)).pdf")
        }
    }
    
    func saveAsDraft() {
        viewModel.saveAsDraft(employeeName: employeeName, employeeId: employeeID, violationDate: violationDate, location: location, description: description, groupData: groupData, image: imagesDatas, reportedBy: reportedBy){ completed in
            if completed {
    ...
    func setViolation(violation: Violation) {
        withAnimation {
            self.reportedBy = violation.reportedBy
            self.isProjectOption = (violation.facilities != nil)
            self.isGroupView = (violation.facilities != nil)
            self.isGroupActive = (violation.facilities != nil)
            self.groupData = violation.facilities
            self.employeeName = violation.employeeName
            self.employeeID = violation.employeeId
            self.violationDate = violation.violationDate.repoDate(inputFormat: Constants.DateFormat.REPO_DATE, local: LocalizationService.shared.language.local)
            self.location = violation.location ?? ""
            self.description = violation.description ?? ""
            self.imagesDatas = violation.images ?? []
            self.imageCount = violation.images?.count ?? 0
        }
    }
    ...
    func savePdf(urlString:String) {
        ...
        let pdfNameFromUrl = "QualityExpertise - Violations(\(viewModel.violation?.id ?? -1)).pdf"
