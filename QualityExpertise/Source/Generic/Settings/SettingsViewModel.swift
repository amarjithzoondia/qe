        ...
        if user.isGuestUser {
            ...
            UpdateRequest
                ...
                .makeCall(responseType: UpdateRequest.CompanyDetailsResponse.self) { (isLoading) in
                    ...
                    guard var userDetailsData = UserManager.getUserDetails() else {
                        let systemError = SystemError("update_company_details_error".localizedString())
                        self.error = systemError
                        self.toast = systemError.toast
                        return
                    }
