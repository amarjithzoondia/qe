...
extension AppDelegate: UNUserNotificationCenterDelegate {
    ...
    func userNotificationCenter(_ center: UNUserNotificationCenter, willPresent notification: UNNotification, withCompletionHandler completionHandler: @escaping (UNNotificationPresentationOptions) -> Void) {
        ...
        if notificationPushType == .notification {
            UserManager.shared.notificationUnReadCount = UserManager.shared.notificationUnReadCount + 1
            NotificationCenter.default.post(name: .UPDATE_COUNT, object: nil)
        } else {
            UserManager.shared.pendingActionsCount = UserManager.shared.pendingActionsCount + 1
            NotificationCenter.default.post(name: .UPDATE_COUNT, object: nil)
        }
    }
