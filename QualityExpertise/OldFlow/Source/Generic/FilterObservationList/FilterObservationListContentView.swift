...
struct FilterObservationListContentView: View {
    ...
    var body: some View {
        ...
        .pickerViewerOverlay(viewerShown: $selectStartDate, title: "Date Open".localizedString()) {
            DatePicker("",
                       selection: Binding<Date>(get: { self.startDate ?? Date()},
                                                set: { self.startDate = $0 }),
                       in: Date()...,
                       displayedComponents: .date)
                .modifier(DatePickerViewStyle())
        }
        .pickerViewerOverlay(viewerShown: $selectEndDate, title: "Date Close".localizedString()) {
            DatePicker("",
                       selection: Binding<Date>(get: { self.endDate ??  Date()}, set: { self.endDate = $0}),
                       in: (startDate ?? Date())...,
                       displayedComponents: .date)
                .modifier(DatePickerViewStyle())
        }
                ...
                VStack(spacing: 0) {
                    HStack(spacing: 10) {
                        ...
                        VStack(spacing: 5) {
                            ...
                            LeftAlignedHStack(
                                Text("Show all Observations that are not specified to any group.")
                                    .foregroundColor(Color.Grey.SLATE)
                                    .font(.regular(13))
                                    .lineLimit(2)
                            )
    ...
    var dateView: some View {
        VStack(spacing: 25) {
            ThemeSelectionFieldView(title: "Date Open".localizedString(),
            ...
            ThemeSelectionFieldView(title: "Date Close".localizedString(),
