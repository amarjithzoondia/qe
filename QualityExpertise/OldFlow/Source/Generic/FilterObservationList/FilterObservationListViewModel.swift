...
class FilterObservationViewModel: BaseViewModel, ObservableObject {
    ...
    var selectedGroupsIds: [Int]? {
        searchGroups.filter({ ($0.isSelected ?? false) }).map({ $0.groupId.toInt! })
    }
    
    var selectedObserversIds: [Int]? {
        searchObserversUsersList.filter({ ($0.isSelected ?? false) }).map({ $0.userId })
    }
    
    var selectedResponsiblePersonsIds: [Int]? {
        searchresponsiblePersonsList.filter({($0.isSelected ?? false)}).map({$0.userId})
    }
